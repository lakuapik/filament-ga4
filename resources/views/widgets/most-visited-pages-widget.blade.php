  ...
  <x-filament::card>
    <div class="flex items-center justify-between gap-8">
      ...
      <select wire:model="filter" @class([
        'text-sm font-medium text-gray-500 border-gray-300 block h-9 transition duration-75 rounded-lg ',
        'shadow-sm focus:border-primary-600 focus:ring-1 focus:ring-inset focus:ring-primary-600',
    ...
    <div class="relative space-y-2 max-auto" wire:init='init'>
                ...
                <div class="flex justify-between items-center">
                  <div class="flex justify-start items-center space-x-1 ">
                    ...
                    <span class="text-sm text-gray-400 group-hover:text-primary-400">
                      <a href="https://{{ $record['hostname'] . $record['path'] }}" target="_blank"
                        class="flex justify-start items-center group-hover:text-primary-500">
                        &NonBreakingSpace;
                        {{ $record['path'] }}
                        &NonBreakingSpace;
                  ...
                  <span class="
                    inline-flex items-center justify-center min-h-6 px-2 py-0.5 text-sm font-medium tracking-tight
                    rounded-xl text-primary-700 bg-primary-500/10 whitespace-normal dark:text-primary-500
                  ">
                    {{ FilamentGA4::thousandsFormater($record['visits']) }}
                  </span>
                </div>
              </div>
            </li>
      ...
      <div wire:loading.delay.long wire:target='filter'>
        <div class="absolute inset-0">
          <div class="flex justify-center items-center mx-auto h-full w-full" wire:loading.delay.long.class='bg-[#ffffffccc]'>
            <x-filament-ga4::loading-indicator />
          </div>
        </div>
      </div>
